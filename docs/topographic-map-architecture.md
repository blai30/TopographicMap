...
PRODUCER (on the orthographic map camera, 2048x2048 SubViewport)
  ...
  RGBA32F segment texture holding (x0,y0,x1,y1) per cell (x0 < 0 means no contour). It is
  twice as wide as the cell grid, storing two segments per cell (primary + secondary): the
  secondary carries a saddle cell's second contour branch. Both are wrapped in Texture2Drds
  so a canvas shader can sample them directly.
...
- `TopographicCompositorEffect.cs` + `depth_to_height.glsl` (height) + `contour_seed.glsl` (per-cell segment): the producer. A `CompositorEffect` at `PreTransparent` that runs the height pass then the seed pass, writing the contour segments into a persistent `RGBA32F` texture it owns, twice as wide as the cell grid so it holds two segments per cell (primary plus a secondary that carries a saddle cell's second branch; created lazily, sized to the buffer, freed on predelete). The height pass writes normalized height + coverage mask into a second persistent `RGBA16F` texture (not the SubViewport color), and both are exposed as `Texture2Drd` properties (`SegmentTexture`, `HeightTexture`) whose RIDs the compositor sets, so a canvas shader can sample them directly. Exported params: `HeightMin`, `HeightMax`, `ContourInterval` (used by the seed pass). The camera rig (height Y, near/far) is not exported: the render callback reads it from `RenderData.GetRenderSceneData()` each frame (`GetCamTransform().Origin.Y`, plus near/far derived from the orthographic projection's z-column, since `GetZNear`/`GetZFar` assume a perspective frustum), so it tracks the actual map camera and cannot drift. The seed pass also bakes `(height_min, height_max, contour_interval, 1.0)` into the segment texture's last texel (the secondary slot of the last cell) so the consumer can read the elevation model back with one `texelFetch`. The compositor is not a `[Tool]`, so it runs only at runtime, never in the editor. It disposes the per-frame RD wrapper objects (`RenderSceneBuffersRD`, `RDUniform`) deterministically so the GC finalizer does not free RIDs off the render thread (Godot #104263), and frees its RIDs on the render thread via `RenderingServer.CallOnRenderThread`, driven early from a node's `_ExitTree` since predelete runs too late at shutdown.
...
- Editing a compute `.glsl` (an `RDShaderFile`, like `contour_seed.glsl`) outside the editor does not recompile its SPIR-V for a non-editor run: the engine loads the previously imported SPIR-V, so the old shader runs and silently mismatches the new code (it cost a fully black map once, when a stale 1-texel seed pass ran against a 2-texel consumer). Re-import first with `godot --headless --import --path .`, or open the editor (which reimports changed sources on focus). A `canvas_item` `.gdshader` is compiled from source text at runtime, so it does not need this; only the imported compute shaders do.
...
- Saddle cells (where one level crosses all four edges of a cell, so the contour has two branches) carry both branches. The seed pass detects the four-crossing case and picks the correct non-crossing pairing with the asymptotic decider (comparing the cell-center value to the level), then writes both segments to the cell's primary and secondary texels; the consumer's distance search tests both. This is why the segment texture is two texels wide per cell.
...
- The compositor effect is the single owner of `HeightMin`/`HeightMax`/`ContourInterval` (plus the camera-rig params). The seed pass bakes them into the segment texture's last texel (the secondary slot of the last cell), and each consumer reads them back with one `texelFetch`. No script pushes them and no value is authored on the material, so producer and consumer always agree with nothing to hand-sync.
