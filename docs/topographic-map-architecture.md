...
- A `ViewportTexture` bound to a consumer's `height_buffer` resolves its `viewport_path` one frame late when a scene is OPENED IN THE EDITOR. On that first frame the map `ColorRect` draws before the texture is ready, logging "Path to node is invalid: '<SubViewport>'" then "Uniforms were never supplied for set (1)"; the texture then resolves and the preview renders. This is editor-only and benign: the running game is unaffected because `PackedScene.instantiate()` builds the whole tree and resolves local resources before the first draw (a direct game run is clean, verified). No `.tscn` format change prevents the burst (re-saving from the editor does not help, and it returns after an editor restart). The deleted `[Tool]` `TopographicMapView` used to suppress it by binding the texture and gating the draw in-editor; the script-free design accepts the one-time warning instead. Author the `ViewportTexture` through the inspector, not by hand in the `.tscn`: the editor-canonical form references the shared segment `.tres` as `Texture2D` (not `Texture2DRD`) and leaves the `ViewportTexture` itself NOT `resource_local_to_scene` (the material carries `resource_local_to_scene`, and the texture inheriting it through the material is what resolves the path in-game).
