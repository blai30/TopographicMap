...
3. **Attach the producer.** Create a `Compositor`, add a `TopographicCompositorEffect`, and assign the compositor to the camera. Set the compositor's `HeightMin`, `HeightMax`, and `ContourInterval`. The camera rig (height, near, far) is read from the camera automatically, so there is nothing to keep in sync. The compositor owns the elevation model and bakes it into the segment texture, so you set the height range and interval here and nowhere else.
4. **Create the shared height and segment textures.** Make two `Texture2DRD` resources, save each as a standalone `.tres`, and assign one to the compositor's `HeightTexture` and the other to its `SegmentTexture`. These same `.tres` files are what each map material samples (step 5), so the producer and every consumer reference the same two resources.
...
The map camera's height, near, and far are read from `RenderSceneData` every frame in the render callback, so the camera rig is never set on the compositor and cannot drift from the camera. The world extent is whatever the orthographic camera frames, so a different terrain size needs only a camera change, not an addon edit.
...
**First-frame caveat.** The compositor renders once at load, and only at runtime (it is not a `[Tool]`, so it never produces in the editor). A map that draws before the producer's first render samples the segment texture with no live RID and fails with "Uniforms were never supplied for set (1)". Guard it with the compositor's `HasProduced` flag: save each map `ColorRect` hidden in the scene (so it does not draw in the editor either) and reveal it from your code once `HasProduced` is true. The demo's `MapUi` does exactly this.
...
| The map shows nothing in the editor (not playing) | By design. The compositor is not a `[Tool]`, so it produces only at runtime, and the consumer maps are saved hidden; the map renders when you press Play, never in the editor viewport. |
